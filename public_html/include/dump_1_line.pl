...
sub csv_dump { # (database handler, name of table
        ...
        # build and prepare the SQL statement - dumping only one line per table for this test version of the code
        my $sql_statement = join(' ', sprintf('SELECT t.* FROM %s t', $table->{name}), $table->{where_clause}, 'AND ROWNUM < 2');
		...
		my $dump = $table->{name};
        open my $fh, ">:encoding(Latin1)", $dump or die "Cannot open temporary output file $dump: $!";
